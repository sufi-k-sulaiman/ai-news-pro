...
export default function StockDetailModal({ stock, isOpen, onClose }) {
    ...
    useEffect(() => {
        if (stock && isOpen && activeNav === 'overview') {
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-2xl border border-gray-200 p-4 md:p-6">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="font-semibold text-gray-900 text-sm md:text-base">Investment Recommendation</h3>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-2 md:gap-4">
                                <div className="bg-red-50 rounded-xl p-2 md:p-4 text-center">
                                    <p className="text-xs md:text-sm text-gray-600">Low Target</p>
                                    <p className="text-lg md:text-xl font-bold text-red-600">${data.priceTargets?.low || (stock.price * 0.8).toFixed(2)}</p>
                                </div>
                                <div className="bg-purple-50 rounded-xl p-2 md:p-4 text-center">
                                    <p className="text-xs md:text-sm text-gray-600">Mid Target</p>
                                    <p className="text-lg md:text-xl font-bold text-purple-600">${data.priceTargets?.mid || (stock.price * 1.15).toFixed(2)}</p>
                                </div>
                                <div className="bg-green-50 rounded-xl p-2 md:p-4 text-center">
                                    <p className="text-xs md:text-sm text-gray-600">High Target</p>
                                    <p className="text-lg md:text-xl font-bold text-green-600">${data.priceTargets?.high || (stock.price * 1.4).toFixed(2)}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                <>
                                    ...
                                    <div className="space-y-2">
                                        {(irData.annualReports || [
                                            { title: `Annual Report 2024`, date: 'February 28, 2025', description: `Comprehensive financial and operational overview for the fiscal year ending December 31, 2024.`, url: `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${stock.ticker}&type=10-K` },
                                            { title: `Annual Report 2023`, date: 'February 28, 2024', description: `Comprehensive financial and operational overview for the fiscal year ending December 31, 2023.`, url: `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${stock.ticker}&type=10-K` },
                                            { title: `Annual Report 2022`, date: 'February 28, 2023', description: `Comprehensive financial and operational overview for the fiscal year ending December 31, 2022.`, url: `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${stock.ticker}&type=10-K` }
                                        ]).map((r, i) => (
                                            <a key={i} href={r.url || `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${stock.ticker}&type=10-K`} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl hover:bg-purple-50 cursor-pointer transition-colors block">
                                                <div className="w-9 h-9 rounded-lg bg-purple-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                                                    <FileText className="w-4 h-4 text-purple-600" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-medium text-gray-900 hover:text-purple-600">{r.title}</p>
                                ...
                                <>
                                    ...
                                    <div className="space-y-2">
                                        {(irData.plStatements || [
                                            ...
                                            <a key={i} href={`https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=${stock.ticker}&type=10-K`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-orange-50 cursor-pointer transition-colors block">
                                                <div className="w-9 h-9 rounded-lg bg-orange-100 flex items-center justify-center flex-shrink-0">
                                                    <DollarSign className="w-4 h-4 text-orange-600" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="font-medium text-gray-900 hover:text-orange-600">{r.year} P&L Statement</p>
