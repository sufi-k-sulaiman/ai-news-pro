    ...
    const handleFileUpload = async (e) => {
        ...
        if (isTextFile) {
            ...
            try {
                ...
                setFileContent(extracted.output?.content || 'PDF content extracted');
            } catch (err) {
                console.error('PDF extraction error:', err);
                setFileContent('[PDF file attached - will be analyzed]');
            }
            ...
            try {
                ...
                setFileContent(extracted.output?.content || 'Document content extracted');
            } catch (err) {
                setFileContent('[Word document attached - will be analyzed]');
            }
        } else {
            try {
                const text = await file.text();
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (selectedModel === 'qwirey') {
                // Qwirey: Full suite with text, images, charts, and web search
...
Also suggest 3 follow-up questions the user might want to ask.`,
                ...
                // Generate actual images (2 to save time)
                const imagePrompts = imagesResponse?.imagePrompts?.slice(0, 4) || [];
                const generatedImages = [];
                
                for (const imgPrompt of imagePrompts.slice(0, 2)) {
                    try {
                        const img = await base44.integrations.Core.GenerateImage({ prompt: imgPrompt });
                        generatedImages.push({ prompt: imgPrompt, url: img.url });
                    } catch (e) {
                ...
                // GPT-4, Claude, Gemini - direct text response (their own style)
                const modelPrompts = {
    ...
    return (
        <div className="min-h-screen bg-white p-4 md:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                {/* Model Selection - Horizontal row with icons */}
                ...
                {/* Main Input Box - Matching the reference design */}
                <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-lg p-5 mb-8">
                    ...
                    {/* Bottom toolbar - matching reference exactly */}
                    <div className="flex items-center justify-end pt-4 border-t border-gray-100 gap-1">
                        ...
                        {/* Submit Arrow - teal/purple rounded button */}
                        <button
